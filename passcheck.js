...
const minNumCheck = /\d*(?=\-)/g
const maxNumCheck = /(?<=\-)\d*/g
const letterCheck = /[a-z](?=\:)/g
const passCheck = /(?<=\:\s).*/g
var validCount = 0

function checkNull(letterGroup, _callback) {
  if (letterGroup) {
    const quantValue = letterGroup.length
    _callback(quantValue)
  } else {
    const quantValue = 0
    _callback(quantValue)
  }
...
function doCheck() {
  for (let i = 0; i < checkData.length; i++) {
    const minNumValue = checkData[i].match(minNumCheck)
    const maxNumValue = checkData[i].match(maxNumCheck)
    const letterValue = checkData[i].match(letterCheck)
    const passValue = checkData[i].match(passCheck)
    const quantCheck = new RegExp(`${letterValue[0]}`, 'g')
    const letterGroup = passValue[0].match(quantCheck)
    checkNull(letterGroup, function (quantValue) {
      if (quantValue >= minNumValue[0] && quantValue <= maxNumValue[0]) {
...
doCheck()
console.log('Final count = ' + validCount)
